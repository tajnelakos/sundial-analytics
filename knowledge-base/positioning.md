...
Used by prompts/skills that generate customer-facing content — see [`sales-deck-builder`](../sales-tools/sales-deck-builder) and [`branding-guideline`](./branding-guideline).
...
## Value propositions mapped to pain, by segment

| Segment | Pain | Value proposition |
|---|---|---|
| Credit Risk (bank/lender) | Can't defend automated decisions to an auditor | A valuation you can explain, not just deliver |
| Mortgage Ops (bank/lender) | Manual review slows down loan cycle time | Minutes, not days, without giving up defensibility |
| Brokerage | Agent tools go unused after initial rollout | Valuation insight that helps agents win the listing conversation, not another dashboard |

## What this is not

Not a claim of superior raw accuracy over every competitor in every scenario — that claim is not reliably defensible and erodes credibility with a technically skeptical buyer (see the [Head of Credit Risk persona](./personas.md)). The narrative wins on defensibility and transparency, not a race to the highest accuracy number.
