...
Canonical voice reference. [`branding-guideline/skill.md`](./branding-guideline/skill.md) applies these rules as a rewrite pass — this file is the source of truth they point back to, so the rules exist once, not duplicated per skill.
...
A rewrite pass should flag, not silently fix, any claim it can't verify from the source material (a specific stat, a comparison) — see the [`branding-guideline`](./branding-guideline) skill for how this gets operationalized.
