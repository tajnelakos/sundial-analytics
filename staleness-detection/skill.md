...
5. **Do not rewrite the flagged claim.** Route hard flags to the appropriate generator skill instead (e.g. a stale competitor claim → `battlecard`; a stale persona claim → note it for `personas.md`'s owner) — this skill's job is detection, not correction.
